...
class Separation(torch.nn.Module):
    ...
    def forward(self, feat, is_eval=False):
        ...
        #print(mask.shape, self.C, self.H, self.W)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, tau=0.1, image_size=(32, 32)):
        ...
        self.rfsize = int(self.image_size[0]/8)
        # self.rfsize = 31
        ...
        # self.separation = Separation(size=(512, 31, 31), tau=self.tau) #分离网络
    ...
    def forward(self, x, y=None, is_eval=False, is_train=False):
        ...
        out1 = out
        feature4 = F.avg_pool2d(out1, self.rfsize).view(out1.size(0), -1)

        r_feat, nr_feat, mask = self.separation(out, is_eval=is_eval)

        feature4_rf = F.avg_pool2d(r_feat, self.rfsize).view(r_feat.size(0), -1)
        feature4_nrf = F.avg_pool2d(nr_feat, self.rfsize).view(nr_feat.size(0), -1)
