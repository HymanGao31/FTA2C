...
class WideResNet(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0, tau=0.1, image_size=(32, 32)):
        ...
        self.norfeaturetran = NoRobustTranformer(size=(640, int(self.image_size[0] / 4), int(self.image_size[1] / 4)))

        self.norfeaturecom = self._make_layer_(BasicBlock_CAS, nChannels[3], nChannels[3], 1, 1, dropRate)
    ...
    def _make_layer_(self, block, in_planes, out_planes, nb_layers, stride, dropRate):
        layers = []
        for i in range(int(nb_layers)-1, int(nb_layers)):
            layers.append(BasicBlock_CAS(in_planes, out_planes, stride, dropRate))
    ...
    def forward(self, x, y=None, is_eval=False, is_train=False):
        ...
        out1 = out
        feature4 = F.avg_pool2d(out1, 8).view(out1.size(0), -1)

        r_feat, nr_feat, mask = self.separation(out, is_eval=is_eval)

        feature4_rf = F.avg_pool2d(r_feat, 8).view(r_feat.size(0), -1)
        feature4_nrf = F.avg_pool2d(nr_feat, 8).view(nr_feat.size(0), -1)
...
def WideResNet34_FTA2C_AA(num_classes=10, tau=0.1, image_size=(32, 32)):
